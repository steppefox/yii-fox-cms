...
class CJsTree extends CWidget
{
    ...
    public function init()
    {
        ...
        $bindings=$this->getClientBindings();
        
        //$cs->registerScript('Yii.CJsTree#'.$id,"$(function () { $(\"#{$id}\").tree($options); });");
        $cs->registerScript('Yii.CJsTree#'.$id,"$(\"#{$id}\")$bindings.jstree($options);");
	...
	protected function getClientOptions()
	{
		$options=$this->options;
                ...
                $optionsParams=array('selected','opened','languages','path','cookies','ui','rules','json_data','lang', 'dnd','crrm', 'plugins');
        ...
        protected function getClientBindings()
        {
            $bindParams = array('select_node', 'loaded'); //TODO: support more callbacks
            $bindingString = "";

            foreach($bindParams as $callback)
            {
                if(array_key_exists($callback, $this->callback))
                {
                    $function = $this->callback[$callback];
                    $bindingString .= ".bind(\"$callback.jstree\", $function)";
                }
            }
