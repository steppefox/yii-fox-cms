...
class WithRelatedBehavior extends CActiveRecordBehavior
{
    ...
    private function internalSave($data=null, $owner=null)
    {
        ...
        try
        {
            ...
            {
                return $owner->delete(); //dont care about relations when deleting
            }
                ...
                switch ($relationClass)
                {
                    ...
                    case CActiveRecord::HAS_MANY:
                        ...
                        foreach ($related as $model)
                        {
                            ...
                            {
                                //Yii::log(get_class($model) . " fk = " . $model->$fk . " " . get_class($owner) . " pk = ". $owner->$pk, 'error');
                                $model->$fk = $owner->$pk;
                            }
                    ...
                    case CActiveRecord::MANY_MANY:
                        ...
                        //MOD: try to delete all links to the related model
                        $criteria = new CDbCriteria;
                        $criteria->compare($fks[0], $owner->primaryKey);
