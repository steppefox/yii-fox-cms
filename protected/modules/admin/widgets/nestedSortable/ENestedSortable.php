...
class ENestedSortable extends CWidget
{
	...
	public $data;
        ...
        public $onupdate; //DEPRECATED
        public $ondelete; //DEPRECATED
        
        public $updateUrl; //DEPRECATED
        public $deleteUrl; //DEPRECATED
	...
	public $cssFile;
         ...
         * Default: drag 20 pixels to the right before it becomes nested
         */
        public $tabSize = 20;
	...
	{
            if(isset($this->buttons['delete']) && !isset($this->buttons['delete']['click']))
                $this->buttons['delete']['click']="js:function() { if(!confirm(".CJavaScript::encode(Yii::t('zii','Are you sure you want to delete this item?')).")) return false; }";

            $html='<div class="tree-buttons">';
            foreach($this->buttons as $id=>$button)
            {
                    $html .= $this->renderButton($id,$button,$node);
            }
            $html .='</div>';
            return $html;
	}
        protected function renderButton($id,$button, $node)
	{
            if (isset($button['visible']) && !$button['visible'])
                    return;

            $label=isset($button['label']) ? $button['label'] : $id;
            $url=isset($button['url']) ? $this->evaluateExpression($button['url'],array('data'=>$node)) : '#';
            $options=isset($button['options']) ? $button['options'] : array('class'=>$id.' button ui-button');
		...
		$cs->registerCssFile($this->cssFile);
            ...
            foreach($this->buttons as $id=>$button)
            {
                if(isset($button['click']))
                {
                    ...
                    $js[]="jQuery('#{$this->id} a.{$id}').live('click',$function);";
                }
            }

            if($js!==array())
                $cs->registerScript(__CLASS__.'#btns'.$this->id, implode("\n",$js));
	}
...
*/
	 ...
	 */
	public function renderTree($data)
	{
		$html='';
		if(is_array($data))
		{
